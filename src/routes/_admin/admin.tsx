...
function AdminConsole() {
  ...
  const loadPool = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/pool?variant=stats", {
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="mt-6">
          <div className="-mx-4 overflow-x-auto px-4 pb-1 md:mx-0 md:px-0">
            <TabsList className="w-max min-w-full">
              ...
              <TabsTrigger value="pool">
                Account Pool
                {Object.values(poolInventory).reduce((a: number, b: number) => a + b, 0) === 0 && (
          ...
          <TabsContent value="pool" className="mt-6 space-y-4">
              ...
              return (
                <Alert variant="default" className="border-warning/40 bg-warning/5">
                  ...
                  <AlertDescription className="text-warning-foreground text-sm">
                    <span className="font-display font-semibold">Low Stock:</span>{" "}
                    {low.map(([size, count]) => `${formatNaira(Number(size))} (${count})`).join(", ")}
                  </AlertDescription>
                </Alert>
              );
            })()}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <Button size="sm" onClick={() => setPoolFormOpen(true)} className="font-display">
              ...
              {challengeList.filter((c: any) => c.is_active).map((c: any) => {
                ...
                return (
                  <div key={c.id} className="rounded-xl border border-border bg-card p-4">
                    ...
                    <div className={`font-display mt-1 text-2xl font-bold ${color}`}>
                      {available}
                    </div>
                    ...
                    {poolAccounts.length === 0 && (
                      <TableRow>
                        <TableCell colSpan={9} className="text-center text-sm text-muted-foreground py-8">
                          {poolLoading ? "Loading…" : "No accounts in pool. Add one to get started."}
                        </TableCell>
                      </TableRow>
                    )}
                    {poolAccounts.map((a: any) => {
                      ...
                      return (
                        <TableRow key={a.id}>
                          ...
                          <TableCell>
                            <Badge variant="outline" className={`font-display ${statusColor[a.status] ?? ""}`}>
                          ...
                          <TableCell className="font-mono text-xs text-muted-foreground">
                            {a.assigned_order_id ? a.assigned_order_id.slice(0, 8) + "…" : "—"}
                          </TableCell>
                          <TableCell className="max-w-[120px] truncate text-xs text-muted-foreground" title={a.notes ?? ""}>
                            {a.notes ?? "—"}
                          </TableCell>
                          <TableCell>
      ...
      <Dialog open={poolFormOpen} onOpenChange={(o) => !poolSaving && !o && setPoolFormOpen(false)}>
        <DialogContent className="mx-4 w-[calc(100%-2rem)] max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the MT5 credentials for a demo account created in the broker terminal.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-3">
            <div className="grid gap-1.5">
              <Label htmlFor="pool-login">MT5 Login *</Label>
              <Input id="pool-login" value={poolForm.mt5_login} onChange={(e) => setPoolForm({ ...poolForm, mt5_login: e.target.value })} placeholder="e.g. 12345678" />
            </div>
            <div className="grid gap-1.5">
              <Label htmlFor="pool-password">Master password *</Label>
              <Input id="pool-password" value={poolForm.mt5_password} onChange={(e) => setPoolForm({ ...poolForm, mt5_password: e.target.value })} placeholder="Trading password" />
            </div>
            <div className="grid gap-1.5">
              <Label htmlFor="pool-investor">Investor password *</Label>
              <Input id="pool-investor" value={poolForm.investor_password} onChange={(e) => setPoolForm({ ...poolForm, investor_password: e.target.value })} placeholder="Read-only password for VPS monitoring" />
            </div>
            <div className="grid gap-1.5">
              <Label htmlFor="pool-server">MT5 Server</Label>
              <Input id="pool-server" value={poolForm.mt5_server} onChange={(e) => setPoolForm({ ...poolForm, mt5_server: e.target.value })} placeholder="Exness-MT5Trial9" />
            </div>
            <div className="grid gap-1.5">
              ...
              <Select value={poolForm.account_size_ngn} onValueChange={(v) => setPoolForm({ ...poolForm, account_size_ngn: v })}>
                <SelectTrigger id="pool-size">
                  <SelectValue placeholder="Select account size" />
                </SelectTrigger>
                <SelectContent>
                  {challengeList.filter((c: any) => c.is_active).map((c: any) => (
                    <SelectItem key={c.id} value={String(c.account_size)}>
                      {c.name} — {formatNaira(c.account_size)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="grid gap-1.5">
              ...
              <Input id="pool-notes" value={poolForm.notes} onChange={(e) => setPoolForm({ ...poolForm, notes: e.target.value })} placeholder="Optional admin notes" />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setPoolFormOpen(false); }} disabled={poolSaving}>
              Cancel
            </Button>
            <Button
              onClick={async () => {
                if (!poolForm.mt5_login.trim() || !poolForm.mt5_password.trim() || !poolForm.investor_password.trim() || !poolForm.account_size_ngn) {
                  toast.error("Login, password, investor password, and account size are required");
                  return;
                }
                setPoolSaving(true);
                try {
                  ...
                  const res = await fetch("/api/admin/pool", {
                    ...
                    body: JSON.stringify({
                      ...
                      account_size_ngn: Number(poolForm.account_size_ngn),
                  ...
                  if (json.ok) {
                    ...
                    setPoolForm({ mt5_login: "", mt5_password: "", investor_password: "", mt5_server: "Exness-MT5Trial9", account_size_ngn: "", notes: "" });
