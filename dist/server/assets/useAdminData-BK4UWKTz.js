...
import { r as rejectKycDocumentServer, a as verifyKycDocumentServer, b as verifyKycServer } from "./kyc.functions-Ddgzg7nS.js";
...
function useAdminDataHook() {
  ...
  const [stats, setStats] = reactExports.useState({ traders: 0, accounts: 0, funded: 0, active: 0, passed: 0, breached: 0, pending: 0, revenue: 0, paid: 0, sold: 0, passRate: 0 });
  const [unprovisionedOrders, setUnprovisionedOrders] = reactExports.useState([]);
  ...
  const [delivering, setDelivering] = reactExports.useState(false);
  const [deliverFor, setDeliverFor] = reactExports.useState(null);
  const [form, setForm] = reactExports.useState({ login: "", password: "", investor: "", server: "" });
  const [tickets, setTickets] = reactExports.useState([]);
  const [selectedTicket, setSelectedTicket] = reactExports.useState(null);
  const [ticketMessages, setTicketMessages] = reactExports.useState([]);
  const [replyText, setReplyText] = reactExports.useState("");
  const [replySaving, setReplySaving] = reactExports.useState(false);
  const [statusFilter, setStatusFilter] = reactExports.useState("all");
  const [statusUpdating, setStatusUpdating] = reactExports.useState(null);
  const [affPayouts, setAffPayouts] = reactExports.useState([]);
  const [freeClaims, setFreeClaims] = reactExports.useState([]);
  const [affSaving, setAffSaving] = reactExports.useState(null);
  const [affiliateStats, setAffiliateStats] = reactExports.useState([]);
  const [affiliateSummary, setAffiliateSummary] = reactExports.useState({ total: 0, referrals: 0, earned: 0, paid: 0, pending: 0, revenue: 0 });
  const [partners, setPartners] = reactExports.useState([]);
  const [partnerPayouts, setPartnerPayouts] = reactExports.useState([]);
  const [partnerSaving, setPartnerSaving] = reactExports.useState(null);
  const [newPartnerEmail, setNewPartnerEmail] = reactExports.useState("");
  const [newPartnerRate, setNewPartnerRate] = reactExports.useState("20");
  const [newPartnerChallengeId, setNewPartnerChallengeId] = reactExports.useState("");
  const [newPartnerPromoCode, setNewPartnerPromoCode] = reactExports.useState("");
  const [addingPartner, setAddingPartner] = reactExports.useState(false);
  const [editingPartner, setEditingPartner] = reactExports.useState(null);
  const [editRateValue, setEditRateValue] = reactExports.useState("");
  const [editChallengeId, setEditChallengeId] = reactExports.useState("");
  const [editPromoCode, setEditPromoCode] = reactExports.useState("");
  const [partnerFreeAccounts, setPartnerFreeAccounts] = reactExports.useState([]);
  const [deliverClaimFor, setDeliverClaimFor] = reactExports.useState(null);
  const [claimForm, setClaimForm] = reactExports.useState({ login: "", password: "", investor: "", server: "" });
  const [deliveringClaim, setDeliveringClaim] = reactExports.useState(false);
  const [deliverPartnerFreeFor, setDeliverPartnerFreeFor] = reactExports.useState(null);
  const [partnerFreeForm, setPartnerFreeForm] = reactExports.useState({ login: "", password: "", investor: "", server: "" });
  const [deliveringPartnerFree, setDeliveringPartnerFree] = reactExports.useState(false);
  const [discountCodes, setDiscountCodes] = reactExports.useState([]);
  const [discountForm, setDiscountForm] = reactExports.useState({ code: "", percent_off: "15", max_redemptions: "", expires_at: "", is_active: true, challenge_id: "" });
  const [discountSaving, setDiscountSaving] = reactExports.useState(null);
  const [tgBotToken, setTgBotToken] = reactExports.useState("");
  const [tgChatId, setTgChatId] = reactExports.useState("");
  const [tgSaving, setTgSaving] = reactExports.useState(false);
  const [tgTesting, setTgTesting] = reactExports.useState(false);
  const [equityDraft, setEquityDraft] = reactExports.useState({});
  const [equitySaving, setEquitySaving] = reactExports.useState(null);
  const [kycTarget, setKycTarget] = reactExports.useState(null);
  const [kycVerifying, setKycVerifying] = reactExports.useState(false);
  const [kycRejectReason, setKycRejectReason] = reactExports.useState("");
  const [kycRejecting, setKycRejecting] = reactExports.useState(false);
  const [breachTarget, setBreachTarget] = reactExports.useState(null);
  const [breachReason, setBreachReason] = reactExports.useState("");
  const [breaching, setBreaching] = reactExports.useState(false);
  const [breachType, setBreachType] = reactExports.useState("inactivity");
  const [breachPair, setBreachPair] = reactExports.useState("");
  const [breachOpenTime, setBreachOpenTime] = reactExports.useState("");
  const [breachCloseTime, setBreachCloseTime] = reactExports.useState("");
  const [breachDuration, setBreachDuration] = reactExports.useState("");
  const [warnTarget, setWarnTarget] = reactExports.useState(null);
  const [warnReason, setWarnReason] = reactExports.useState("");
  const [warnType, setWarnType] = reactExports.useState("inactivity");
  const [warnPair, setWarnPair] = reactExports.useState("");
  const [warnOpenTime, setWarnOpenTime] = reactExports.useState("");
  const [warnCloseTime, setWarnCloseTime] = reactExports.useState("");
  const [warnDuration, setWarnDuration] = reactExports.useState("");
  const [warning, setWarning] = reactExports.useState(false);
  ...
  const [rejectType, setRejectType] = reactExports.useState(null);
  const [payoutRejectTarget, setPayoutRejectTarget] = reactExports.useState(null);
  const [payoutRejectReason, setPayoutRejectReason] = reactExports.useState("");
  const [payoutRejecting, setPayoutRejecting] = reactExports.useState(false);
  const [poolAccounts, setPoolAccounts] = reactExports.useState([]);
  const [poolInventory, setPoolInventory] = reactExports.useState({});
  const [poolLoading, setPoolLoading] = reactExports.useState(false);
  const [poolFormOpen, setPoolFormOpen] = reactExports.useState(false);
  const [poolSaving, setPoolSaving] = reactExports.useState(false);
  const [viewCredsFor, setViewCredsFor] = reactExports.useState(null);
  const [poolForm, setPoolForm] = reactExports.useState({ mt5_login: "", mt5_password: "", investor_password: "", mt5_server: "Exness-MT5Trial9", account_size_ngn: "", account_size_usd: "", currency: "NGN", notes: "" });
  const [socialItems, setSocialItems] = reactExports.useState([]);
  const [uploadFile, setUploadFile] = reactExports.useState(null);
  const [uploadPreview, setUploadPreview] = reactExports.useState("");
  const [uploadLabel, setUploadLabel] = reactExports.useState("");
  const [uploadCategory, setUploadCategory] = reactExports.useState("payout");
  const [uploadOrder, setUploadOrder] = reactExports.useState("0");
  const [uploading, setUploading] = reactExports.useState(false);
  const [savingSocialOrder, setSavingSocialOrder] = reactExports.useState(null);
  const [socialDeleting, setSocialDeleting] = reactExports.useState(null);
  const [challengeList, setChallengeList] = reactExports.useState([]);
  const [challengeEditOpen, setChallengeEditOpen] = reactExports.useState(false);
  const [editingChallenge, setEditingChallenge] = reactExports.useState(null);
  const [challengeForm, setChallengeForm] = reactExports.useState(blankChallenge);
  const [savingChallenge, setSavingChallenge] = reactExports.useState(false);
  ...
  const saveChallenge = async () => {
    ...
    const payload = {
      ...
      phase2_profit_target_percent: Number(challengeForm.phase2_profit_target_percent) || null,
  ...
  const toggleChallengeActive = async (c) => {
  ...
  const [deletingChallengeId, setDeletingChallengeId] = reactExports.useState(null);
  ...
  const load = async () => {
    ...
    const accRows = accRaw.data ?? [];
    const poRows = poRaw.data ?? [];
    const reqRows = req.data ?? [];
    const userIds = Array.from(/* @__PURE__ */ new Set([...accRows.map((a) => a.user_id), ...poRows.map((p) => p.user_id), ...reqRows.map((r) => r.user_id)]));
    const challengeIds = Array.from(/* @__PURE__ */ new Set([...accRows.map((a) => a.challenge_id), ...reqRows.map((r) => r.challenge_id), ...(ord.data ?? []).map((o) => o.challenge_id)]));
    ...
    const [profRes, chRes, ordRes, taRes] = await Promise.all([
      userIds.length ? supabase.from("profiles").select("id, full_name, bank_account_number, bank_name, bank_account_name, kyc_verified").in("id", userIds) : Promise.resolve({ data: [] }),
      challengeIds.length ? supabase.from("challenges").select("id, name, account_size, profit_target_percent, phase2_profit_target_percent, max_drawdown_percent, phases").in("id", challengeIds) : Promise.resolve({ data: [] }),
      orderIds.length ? supabase.from("orders").select("id, status").in("id", orderIds) : Promise.resolve({ data: [] }),
      accountIds.length ? supabase.from("trader_accounts").select("id, mt5_login").in("id", accountIds) : Promise.resolve({ data: [] })
    ]);
    const profMap = new Map((profRes.data ?? []).map((p) => [p.id, p]));
    const chMap = new Map((chRes.data ?? []).map((c) => [c.id, c]));
    const ordMap = new Map((ordRes.data ?? []).map((o) => [o.id, o]));
    const taMap = new Map((taRes.data ?? []).map((t) => [t.id, t]));
    const accList = accRows.map((a) => ({ ...a, profiles: profMap.get(a.user_id) ?? null, challenges: chMap.get(a.challenge_id) ?? null, _trading_days: a.trading_days ?? 0 }));
    const accMap = new Map(accList.map((a) => [a.id, a]));
    const poList = poRows.map((p) => ({ ...p, profiles: profMap.get(p.user_id) ?? null, trader_accounts: accMap.get(p.trader_account_id) ?? taMap.get(p.trader_account_id) ?? null }));
    const hydrated = reqRows.map((r) => ({ ...r, profiles: profMap.get(r.user_id) ?? null, challenges: chMap.get(r.challenge_id) ?? null, orders: ordMap.get(r.order_id) ?? null }));
    accList.sort((a, b) => {
      const score = (x) => {
    ...
    const ordersList = ord.data ?? [];
    const soldOrders = ordersList.filter((o) => o.status === "paid" || o.status === "delivered");
    const soldCount = soldOrders.length;
    const soldValue = soldOrders.reduce((s, o) => s + Number(chMap.get(o.challenge_id)?.account_size ?? 0), 0);
    ...
    setStats({
      ...
      revenue: ordersList.filter((o) => o.status === "paid" || o.status === "delivered").reduce((s, o) => s + Number(o.amount_paid), 0) / 100,
      paid: poList.filter((p) => p.status === "paid").reduce((s, p) => s + Number(p.amount_naira), 0),
      passRate
    });
    try {
      const { data } = await supabase.rpc("find_unprovisioned_orders");
      setUnprovisionedOrders(data ?? []);
    } catch {
      setUnprovisionedOrders([]);
    }
  };
  const loadPool = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/admin/pool?variant=stats", { headers: { Authorization: `Bearer ${session2.access_token}` } });
      const json = await res.json();
      if (json.ok) {
        setPoolInventory(json.inventory ?? {});
        setPoolAccounts(json.rows ?? []);
      }
    } catch (e) {
      console.error("[admin] loadPool failed", e);
    } finally {
...
Admin replied to ticket.` });
  ...
  const updateTicketStatus = async (t, newStatus) => {
...
Status changed to ${newStatus.replace("_", " ")}.` });
    ...
    if (selectedTicket?.id === t.id) setSelectedTicket((prev) => prev ? { ...prev, status: newStatus } : null);
    loadTickets();
  };
  const statusFlow = { open: ["in_progress"], in_progress: ["resolved"], resolved: ["open"] };
  ...
  const updatePayout = async (p, status) => {
    const { error } = await supabase.from("payouts").update({ status, processed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", p.id);
    if (error) return toast.error(error.message);
    ...
    if (status === "paid") {
      ...
      if (account) {
        await supabase.from("trader_accounts").update({ current_equity: account.starting_balance, peak_equity: account.starting_balance, daily_peak_equity: account.starting_balance, daily_peak_date: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10), trading_days: 0 }).eq("id", account.id);
        await supabase.from("account_snapshots").insert({ trader_account_id: account.id, equity: account.starting_balance, balance: account.starting_balance, profit: 0, drawdown_percent: 0, snapshot_time: (/* @__PURE__ */ new Date()).toISOString() });
  ...
  const resetAccountBalance = async (account) => {
  ...
  const approvePhase2 = async (a) => {
    ...
    const phasePassedAt = new Date(Date.now() - 1e3).toISOString();
    const resetSnapshotAt = (/* @__PURE__ */ new Date()).toISOString();
    const { error } = await supabase.from("trader_accounts").update({ current_phase: 2, current_equity: a.starting_balance, peak_equity: a.starting_balance, daily_peak_equity: a.starting_balance, daily_peak_date: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10), phase1_passed_at: phasePassedAt, phase2_requested_at: null, phase_rejected_reason: null, phase_rejected_at: null, status: "active", trading_days: 0 }).eq("id", a.id);
    if (error) return toast.error(error.message);
    await supabase.from("account_snapshots").insert({ trader_account_id: a.id, equity: a.starting_balance, balance: a.starting_balance, profit: 0, drawdown_percent: 0, snapshot_time: resetSnapshotAt });
    await supabase.from("notifications").insert({ user_id: a.user_id, title: "🎯 Phase 1 Passed", message: "Congratulations — you're now in Phase 2.", type: "success" });
    toast.success("Phase 2 approved");
    notifyEmail({ type: "phase1_passed", accountId: a.id });
    load();
  };
  const approveFunded = async (a) => {
    ...
    const phasePassedAt = new Date(Date.now() - 1e3).toISOString();
    const resetSnapshotAt = (/* @__PURE__ */ new Date()).toISOString();
    const { error } = await supabase.from("trader_accounts").update({ status: "funded", current_equity: a.starting_balance, peak_equity: a.starting_balance, daily_peak_equity: a.starting_balance, daily_peak_date: (/* @__PURE__ */ new Date()).toISOString().slice(0, 10), phase2_passed_at: phasePassedAt, funded_at: phasePassedAt, funded_requested_at: null, phase_rejected_reason: null, phase_rejected_at: null, trading_days: 0 }).eq("id", a.id);
    if (error) return toast.error(error.message);
    await supabase.from("account_snapshots").insert({ trader_account_id: a.id, equity: a.starting_balance, balance: a.starting_balance, profit: 0, drawdown_percent: 0, snapshot_time: resetSnapshotAt });
    await supabase.from("notifications").insert({ user_id: a.user_id, title: "🏆 You're Funded!", message: "Congratulations — your account is now funded.", type: "success" });
    toast.success("Account funded");
    notifyEmail({ type: "funded", accountId: a.id });
    load();
  };
  const submitEquity = async (account) => {
    ...
    const { error } = await supabase.from("account_snapshots").insert({ trader_account_id: account.id, equity, balance: equity, profit, drawdown_percent: Number(drawdown.toFixed(2)) });
  ...
  const openKycVerify = (account) => {
    setKycTarget(account);
    setKycRejectReason("");
  };
  const submitKycVerify = async () => {
    ...
    try {
      ...
      if (hasDoc) {
        ...
        if (!accountNumber) {
          toast.error("Trader hasn't submitted bank details or KYC document");
  ...
  const openBreachDialog = (account) => {
...
— FundedNG Team`);
  ...
  const openWarningDialog = (account) => {
    const name = account?.profiles?.full_name ?? "Trader";
...
— FundedNG Team`);
  ...
  const openRejectDialog = (account, type) => {
    setRejectTarget(account);
    setRejectType(type);
    setRejectReason("");
  };
  const openPayoutRejectDialog = (payout) => {
    setPayoutRejectTarget(payout);
  ...
  const submitPayoutReject = async () => {
    ...
    if (reason.length < 3) {
      toast.error("Please write a reason (min 3 chars).");
      return;
    }
    setPayoutRejecting(true);
    try {
      const { error } = await supabase.from("payouts").update({ status: "rejected", processed_at: (/* @__PURE__ */ new Date()).toISOString(), admin_note: reason }).eq("id", payoutRejectTarget.id);
      if (error) {
  ...
  const submitRejectPhase = async () => {
    ...
    try {
      ...
      const patch = { ...isPhase2 ? { phase2_requested_at: null } : { funded_requested_at: null }, phase_rejected_reason: reason, phase_rejected_at: (/* @__PURE__ */ new Date()).toISOString() };
      const { error } = await supabase.from("trader_accounts").update(patch).eq("id", rejectTarget.id);
      if (error) {
        toast.error(error.message);
        return;
      }
      const phaseLabel = isPhase2 ? "Phase 2" : "Funded";
      await supabase.from("notifications").insert({ user_id: rejectTarget.user_id, title: `❌ ${phaseLabel} Request Rejected`, message: `Your ${phaseLabel} request for account ${rejectTarget.mt5_login} has been rejected. Reason: ${reason}`, type: "error" });
  ...
  const submitBreach = async () => {
    ...
    if (reason.length < 3) {
      toast.error("Please write a breach reason (min 3 chars).");
      return;
    }
    setBreaching(true);
    try {
      const { error } = await supabase.from("trader_accounts").update({ status: "breached", breach_reason: reason, phase_rejected_reason: null, phase_rejected_at: null }).eq("id", breachTarget.id);
      if (error) {
        toast.error(error.message);
        return;
      }
      const adminName = profile?.full_name && profile.full_name.trim() || (user?.email ?? null);
      await supabase.from("breach_audit_log").insert({ trader_account_id: breachTarget.id, user_id: breachTarget.user_id, admin_id: user?.id ?? null, admin_name: adminName, admin_email: user?.email ?? null, reason, mt5_login: breachTarget.mt5_login ?? null }).then(({ error: e }) => {
        if (e) console.error("[breach audit log] insert failed", e.message);
      });
      await supabase.from("notifications").insert({ user_id: breachTarget.user_id, title: "❌ Account breached", message: `Your account ${breachTarget.mt5_login} has been marked as breached. Reason: ${reason}`, type: "error" });
  ...
  const submitWarning = async () => {
    ...
    if (reason.length < 3) {
      toast.error("Please write a warning reason (min 3 chars).");
      return;
    }
    setWarning(true);
    try {
      const { error } = await supabase.from("notifications").insert({ user_id: warnTarget.user_id, title: "⚠️ Trading Warning", message: `Warning for account ${warnTarget.mt5_login}: ${reason}`, type: "warning" });
      if (error) {
        toast.error(error.message);
        return;
      }
  ...
  const loadAffiliate = () => {
    (async () => {
      ...
      const payRows = pRes.data ?? [];
      const claimRows = cRes.data ?? [];
      ...
      const allUserIds = Array.from(/* @__PURE__ */ new Set([...payRows.map((r) => r.user_id), ...claimRows.map((r) => r.affiliate_id), ...affRows.map((r) => r.user_id), ...refRows.map((r) => [r.referrer_id, r.referred_user_id]).flat()]));
      ...
      const chMap = new Map(chRows.map((c) => [c.id, c]));
      const refByAff = /* @__PURE__ */ new Map();
      const commByAff = /* @__PURE__ */ new Map();
      ...
      const statsArr = affRows.map((a) => {
        ...
        const revenue = paidOrders.reduce((s, o) => s + Number(o.amount_paid) / 100, 0);
        const accountSize = paidOrders.reduce((s, o) => {
          const ch = chMap.get(o.challenge_id);
          return s + (ch ? Number(ch.account_size) : 0);
        }, 0);
        return { ...a, profile: profMap.get(a.user_id) ?? null, referralCount: refs.length, paidReferralCount: refs.filter((r) => r.first_paid_at).length, pendingCommissions: comms.filter((c) => c.status === "pending").reduce((s, c) => s + Number(c.amount_naira), 0), totalRevenue: revenue, totalAccountSize: accountSize, ordersCount: paidOrders.length };
      });
      const filtered = statsArr.filter((a) => a.paidReferralCount > 0 || a.ordersCount > 0);
      filtered.sort((a, b) => b.totalRevenue - a.totalRevenue);
      setAffiliateStats(filtered);
      setAffiliateSummary({ total: filtered.length, referrals: filtered.reduce((s, a) => s + a.paidReferralCount, 0), earned: filtered.reduce((s, a) => s + Number(a.total_earned_naira), 0), paid: filtered.reduce((s, a) => s + Number(a.total_paid_naira), 0), pending: filtered.reduce((s, a) => s + a.pendingCommissions, 0), revenue: filtered.reduce((s, a) => s + a.totalRevenue, 0) });
    })();
  };
  const loadPartners = () => {
    (async () => {
      const [pRes, payRes, freeRes] = await Promise.all([
        ...
        supabase.from("partner_free_accounts").select("*, challenges(name, account_size, profit_target_percent, max_drawdown_percent, phases, currency)").order("requested_at", { ascending: false })
      ]);
      const partnerRows = pRes.data ?? [];
      const payRows = payRes.data ?? [];
      const freeRows = freeRes.data ?? [];
      const userIds = Array.from(/* @__PURE__ */ new Set([...partnerRows.map((r) => r.user_id), ...payRows.map((r) => r.partner_id), ...freeRows.map((r) => r.partner_id)]));
      ...
      if (challengeIds.length) {
        const { data: chs } = await supabase.from("challenges").select("id, name, account_size, currency").in("id", challengeIds);
        (chs ?? []).forEach((c) => chMap.set(c.id, c));
      }
      setPartners(partnerRows.map((r) => ({ ...r, profiles: profMap.get(r.user_id) ?? null, free_challenge: r.free_account_challenge_id ? chMap.get(r.free_account_challenge_id) ?? null : null })));
  ...
  const addPartner = async () => {
    ...
    if (userId) {
      const updates = { free_account_challenge_id: challengeVal || null };
      const customCode = newPartnerPromoCode.trim();
      if (customCode) {
        updates.promo_code = customCode.toUpperCase();
      }
      const { error: upErr } = await supabase.from("partner_profiles").update(updates).eq("user_id", userId);
  ...
  const saveCommissionRate = async () => {
    ...
    if (codeChanged) {
      ...
      if (customCode.length < 3) {
        ...
        return toast.error("Promo code must be at least 3 characters");
      }
      ...
      if (codeErr) {
        setPartnerSaving(null);
        return toast.error(codeErr.message);
      }
    }
    const { error } = await supabase.from("partner_profiles").update(updates).eq("id", editingPartner.id);
    setPartnerSaving(null);
    if (error) return toast.error(error.message);
  ...
  const togglePartnerActive = async (p) => {
    ...
    const { error } = await supabase.from("partner_profiles").update({ is_active: !p.is_active }).eq("id", p.id);
    setPartnerSaving(null);
    if (error) return toast.error(error.message);
  ...
  const deletePartner = async (p) => {
    ...
    setPartnerSaving(p.id);
    const { error } = await supabase.rpc("delete_partner_role", { _partner_profile_id: p.id });
    setPartnerSaving(null);
    if (error) return toast.error(error.message);
    toast.success("Partner deleted");
    loadPartners();
  };
  const setPartnerPayoutStatus = async (id, status) => {
    setPartnerSaving(id);
    const { error } = await supabase.from("partner_payouts").update({ status }).eq("id", id);
    setPartnerSaving(null);
    if (error) return toast.error(error.message);
    toast.success(`Marked ${status}`);
    loadPartners();
  };
  const openDeliverPartnerFree = (claim) => {
  ...
  const setAffPayoutStatus = async (id, status) => {
    setAffSaving(id);
  ...
  const setFreeClaimStatus = async (id, status) => {
    setAffSaving(id);
    const { error } = await supabase.from("affiliate_free_accounts").update({ status }).eq("id", id);
    setAffSaving(null);
    if (error) return toast.error(error.message);
    toast.success(`Marked ${status}`);
    loadAffiliate();
  };
  const openDeliverClaim = (claim) => {
  ...
  const submitDeliverClaim = async () => {
    ...
    const { error } = await supabase.from("affiliate_free_accounts").update({ status: "fulfilled", mt5_login: claimForm.login.trim(), mt5_password: claimForm.password.trim(), investor_password: claimForm.investor.trim() || null, mt5_server: claimForm.server.trim(), fulfilled_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", deliverClaimFor.id);
...
function AdminDataProvider({ children }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(AdminDataContext.Provider, { value: data, children });
}
function useAdminData() {
  const ctx = reactExports.useContext(AdminDataContext);
  if (!ctx) throw new Error("useAdminData must be used within an AdminDataProvider");
  return ctx;
}
export {
  AdminDataProvider as A,
  useAdminData as u
};
